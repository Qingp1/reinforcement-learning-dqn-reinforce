...
class ReinforceWithBaseline(tf.keras.Model):
    ...
    def loss_func(self, states: tf.Tensor, actions: List[int], discounted_rewards: List[float]) -> tf.Tensor:
        ...
        The actor loss weights log-probabilities by the advantage (discounted returns minus baseline value).
        tf.stop_gradient prevents the actor loss from updating the critic network.
        The critic loss is MSE between predicted values and discounted returns.
